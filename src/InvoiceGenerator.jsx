...
function validateTIN(tin) {
  if (!tin) return null;
  return /^\d{9}$/.test(tin) ? null : "TIN must be exactly 9 digits";
}
function fmtDate(d) {
  if (!d) return "—";
...
function TINInput({ value, onChange, placeholder }) {
  ...
  return (
    <FieldGroup label="TIN (9 digits)" error={value ? err : null}>
      <input className="ig-field" value={value}
...
function LineItemRow({ item, sym, onUpdate, onRemove }) {
  ...
  return (
    <div className="ig-item-row">
      <input className="ig-item-field" placeholder="Describe item clearly…"
        value={item.desc} onChange={e => onUpdate(item.id,"desc",e.target.value)} />
      <input className="ig-item-field num" type="number" min="0" value={item.qty}
        onChange={e => onUpdate(item.id,"qty",e.target.value)} />
      <select className="ig-item-select" value={item.unit}
        onChange={e => onUpdate(item.id,"unit",e.target.value)}>
        {UNITS.map(u => <option key={u} value={u}>{u}</option>)}
      </select>
      <input className="ig-item-field num" type="number" min="0" step="0.01" value={item.rate}
        onChange={e => onUpdate(item.id,"rate",e.target.value)} />
      <span className="ig-item-amount">{fmt(sym, amount)}</span>
      <button className="ig-remove-btn no-print" onClick={() => onRemove(item.id)} aria-label="Remove">✕</button>
    </div>
  );
}

// ── print-only line item (plain spans, no inputs) ──────────────────────────
function PrintLineItemRow({ item, sym }) {
  const amount = (parseFloat(item.qty)||0) * (parseFloat(item.rate)||0);
  if (!item.desc && !amount) return null;
  return (
    <div className="pi-item-row">
      ...
      <span className="pi-item-num">{fmt(sym, amount)}</span>
    </div>
  );
}
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        /* ═══════════════════════════════════════════════
           PRINT STYLES — hide form, show clean invoice
        ═══════════════════════════════════════════════ */
        @media print {
          /* remove browser header/footer (URL, date, title) */
          @page {
            size: A4;
            margin: 14mm 14mm 14mm 14mm;
          }

          /* hide everything on screen except the print invoice */
          body { background: white !important; margin: 0; }
          .ig-page, .ig-card { display: block !important; max-width: 100% !important; box-shadow: none !important; border-radius: 0 !important; padding: 0 !important; margin: 0 !important; }
          .ig-toolbar,
          .ig-compliance-bar,
          .no-print,
          .ig-remove-btn,
          ...
          .ig-optional-grid,
          .ig-select-field,
          .ig-compliance-footer { display: none !important; }

          /* hide the editable form paper */
          .ig-paper { display: none !important; }

          /* show the print invoice */
          .pi-invoice { display: block !important; }
        }
        ...
        .pi-logo-placeholder { width: 140px; }
        .pi-title-block { text-align: right; }
        .pi-badge { display: inline-block; background: #0f2a5c; color: white; font-size: 17px; font-weight: 700; letter-spacing: 0.12em; padding: 5px 14px; border-radius: 5px; margin-bottom: 10px; }
        ...
        .pi-meta-val { font-weight: 600; min-width: 120px; text-align: right; }

        /* divider */
        .pi-divider { height: 2px; background: #0f2a5c; margin: 12px 0; }
        .pi-divider-thin { height: 1px; background: #c2ccdf; margin: 10px 0; }
        ...
        /* items table */
        .pi-items-header { display: grid; grid-template-columns: 3fr 55px 65px 100px 95px; gap: 6px; padding: 5px 0; border-bottom: 1.5px solid #0f2a5c; margin-bottom: 4px; }
        ...
        .pi-item-row { display: grid; grid-template-columns: 3fr 55px 65px 100px 95px; gap: 6px; padding: 4px 0; border-bottom: 1px solid #e2e8f0; align-items: center; }
        ...
        /* totals */
        .pi-bottom { display: grid; grid-template-columns: 1fr 230px; gap: 20px; margin-top: 14px; }
        ...
        .pi-words { margin-top: 10px; background: #f0f4fb; border-left: 3px solid #0f2a5c; padding: 6px 10px; font-size: 10.5px; color: #334155; border-radius: 0 4px 4px 0; }
        ...
        .pi-footer { margin-top: 16px; border-top: 1px dashed #c2ccdf; padding-top: 10px; font-size: 9.5px; color: #94a3b8; text-align: center; line-height: 1.6; }
      `}</style>
        ...
        <div className="ig-card">
          <div className="ig-toolbar">
            ...
            <div className="ig-toolbar-right">
              ...
              <button className="ig-print-btn" onClick={() => window.print()}>🖨 Save PDF</button>
            </div>
          </div>
          ...
          <div className="ig-paper">
            ...
            <div className="ig-parties">
              <div>
                ...
                <FieldGroup label="Contact">
                  <input className="ig-field" value={supplierContact} onChange={e => setSupplierContact(e.target.value)} placeholder="Email / Phone" />
                </FieldGroup>
              </div>
              <div>
            ...
            <button className="ig-add-btn no-print" onClick={addItem}>+ Add Line Item</button>
      ...
      <div className="pi-invoice">
        ...
        <div className="pi-header">
          <div>
            {logoSrc
              ? <img className="pi-logo" src={logoSrc} alt="logo" />
          ...
          <div className="pi-title-block">
            ...
            <div className="pi-meta">
              <div className="pi-meta-row">
                <span className="pi-meta-label">Invoice No.:</span>
                <span className="pi-meta-val">{fullSerial || "—"}</span>
              </div>
              <div className="pi-meta-row">
                <span className="pi-meta-label">Invoice Date:</span>
                <span className="pi-meta-val">{fmtDate(invoiceDate)}</span>
              </div>
              <div className="pi-meta-row">
                <span className="pi-meta-label">Date of Supply:</span>
                <span className="pi-meta-val">{fmtDate(supplyDate)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="pi-parties">
          <div>
            ...
            {supplierContact && <div className="pi-party-addr">{supplierContact}</div>}
          </div>
          <div>
        ...
        <div className="pi-items-header">
          ...
          <span>Unit</span>
          <span className="r">Rate ({currencyCode})</span>
          <span className="r">Amount ({currencyCode})</span>
        </div>
        {items.map(item => <PrintLineItemRow key={item.id} item={item} sym={sym} />)}
        ...
        <div className="pi-bottom">
          <div>
            {(notes || paymentMode || placeOfSupply) && <>
              ...
              {paymentMode  && <div className="pi-opt-row" style={{ marginTop:8 }}><span className="pi-opt-label">Payment: </span>{paymentMode}</div>}
